...
ONLY through ``store.get_data`` (no peeking) and combines three classic momentum
...
outside. The three sub-scores are combined ``0.5·mom + 0.3·trend + 0.2·slope``.
This per-ticker mapping is monotonic in momentum strength. A cross-sectional
*percentile* (the §8 alternative) needs the universe cross-section and so belongs
to the stats/calibration layer, not this pure per-ticker function.
...
TREND_BAND = (-0.2, 0.2)
SLOPE_BAND = (-0.1, 0.1)
WEIGHTS = {"mom_12_1": 0.5, "price_vs_sma200": 0.3, "sma200_slope": 0.2}
